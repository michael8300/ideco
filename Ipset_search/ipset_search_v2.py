...
def process_file(file_path, ips_to_check):
    ...
    # dataframe_ipset = dataframe_ipset.drop_duplicates(subset=['ip'])
...
def check_ip_in_ipset(dataframe_ipset, ips_to_check, name):
    ...
    for ip in ips_to_check:
        ...
        if match.any():
            matches = (dataframe_ipset[match][['ip', 'prefix_len', 'port']].drop_duplicates(subset=['ip', 'prefix_len']).set_index(['ip', 'prefix_len']).to_dict(orient='index'))
            for (ip_val, prefix_len), data in matches.items():
                ip_with_mask = f"{ip_val}/{prefix_len}"
                final_results.setdefault(name, {})[ip_with_mask] = data['port']
...
def main():
    ...
    directory_path = r'' # Папка с айписетами
    check_file_path = r'' # txt файл с перечнем ip для проверки
